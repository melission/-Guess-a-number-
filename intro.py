...
def critical_hit(hit):
    critical_rate = random.randint(0, 100)
    if critical_rate >= 90:
        return hit*5
    if critical_rate >= 50:
        return hit*3
...
while True:
    ...
    while True:
        ...
        while True:
            ...
            if hit > 0 and hit < 11:
                try_crit = critical_hit(hit)
                if try_crit > hit:
                    print('Удача улыбнулась тебе, усилив атаку на {}, общий урон: {}'.format(try_crit // hit, try_crit))
                    hit = try_crit
            else:
            ...
            while True:
                ...
                try_crit = critical_hit(bothit)
                if try_crit > bothit:
                    print('Удача улыбнулась боту, усилив атаку на {}, общий урон: {}'.format(try_crit // bothit,
                                                                                             try_crit))
                    bothit = try_crit
                hp = hp - bothit
                if hp > 0:
